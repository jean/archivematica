...
def parseDmdSec(dmdSec, label='[Placeholder title]'):
    ...
    if dmdSec is None:
        return {'title': [label]}
    elementsDict = collections.OrderedDict()
    ...
    # TODO check for problems with encoding & unicode vs str
    ...
    for element in xmldata.iterdescendants():
        ...
        if tagname in ('dublincore', ):
            continue
        if tagname in elementsDict:
            # Append to existing list
            elementsDict[tagname].append(element.text or '')
        else:
            # Create new list
            elementsDict[tagname] = [element.text or '']
    ...
    with open(csv_path, "wb") as csv_file:
        ...
        for div in divs_with_dmdsecs:
            ...
            if csv_header_ref and csv_header_ref != csv_header:
                print >> sys.stderr, 'ERROR headers differ,', csv_path, 'almost certainly invalid'
                print >> sys.stderr, 'Refernce header:', csv_header_ref
                print >> sys.stderr, 'Differing header:', csv_header
